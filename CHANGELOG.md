...
- **changelog:** `v0.1.0` ([e1bdb33][2])

#### Build System

- **just/git:** prune refs in `git-fetch` recipe ([69d5e05][3])

## [v0.1.0][4] (2023-02-13)

### Features

- **sdk/api/response:** `WriteJSON()` function implementation ([ac8c96c][5])
- **sdk/api/response:** `WriteSuccessfulJSONRaw()` function implementation
  ([f264be1][6])
- **api/handlers:** remove `Prefix` from pre-flight handler ([ae48e9d][7])
- **api/handlers:** Register and initialize `kubernetes-liveness-probe` route
  ([1c416f5][8])
- **api/handlers/liveness:** `Initialize()` function ([73548a0][9])
- **api/handlers/liveness:** `New()` function ([016f07c][10])
- **api/handlers/liveness:** `Handler` struct ([133a5a8][11])
- **api/handlers/liveness:** package constants ([a0b8f71][12])
- **api/handlers/liveness:** `Response` struct ([9bc1341][13])

### Bug Fixes

- **sdk/api/response:** address linter complaints ([a9eed4e][14])

### Documentation

- **docker/buildx:** `gitpod` target ([11b8055][15])
- `liveness-probe` instruction ([fc5852c][16])
- **sdk/api/response:** `WriteJSON()` function synopsis ([7e25e0a][17])
- **sdk/api/response:** `WriteSuccessfulJSONRaw()` synopsis ([63d16c4][18])
- **swaggger:** add `GET` verb for `/healthz` ([5a0bb54][19])
- fix instruction to run mage's `build` target ([d35f513][20])
- **api/handlers/liveness:** `Initialize()` function ([a8993b7][21])
- **api/handlers/liveness:** `New()` function ([add5240][22])
- **api/handlers/liveness:** `Handler` struct ([06f7e41][23])
- **api/handlers/liveness:** package constants ([0e5f809][24])
- **api/handlers/liveness:** `Status` enum ([2ac0c5e][25])
- **api/handlers/liveness:** package synopsis ([efe22ac][26])
- **changelog:** updated changelog for v0.0.1 ([533f465][27])

### Refactors

- **api/handlers/liveness:** update `handler()` to write response json without
  adding additional fields ([985c2cf][28])
...
- **go-releaser:** do not sign archive ([b931100][31])
- **docker:** `gitpod` target ([006af3c][32])
- **just/bootstrap:** add `rust` toolchain ([6d9e725][33])
- **docker:** laxer package versioning in dockerfile ([9db2bb8][34])
- **docker:** fixed minor issues with `docker-bake.hcl` file ([747b4f4][35])
- **just/bootstrap:** add `bootstrap-os-pkgs` dependency to `bootstrap` target
  ([c397414][36])
- **just/go:** fix `mage` install in `_build-go` target ([7a6a5d9][37])
- **just/commit:** fix `bootstrap-pre-commit` hook installation ([495f420][38])
- **just/semver:** changed changelog commit message pattern ([9a5964e][39])

### CI

- **docker:** detect `TAG` ([5ca9bde][40])
- **go:** add `go-releaser` step ([74090a2][41])
- **release:** fix deprecated flags in `go-releaser` step ([0c641be][42])
- update min go version to `1.20` ([385d829][43])
- **go:** fixed arguments in `test` step ([2e45224][44])
- **go:** fixed arguments in `build` step ([55e6ae7][45])
- **release:** remove `unshallow` step ([2011e4f][46])
- **release:** update `setup-go` and `goreleaser-action` actions
  ([ea4cc92][47])

### Chore

- **gitpod:** source `gitpod` variables ([168e63f][48])
- **gitpod:** `rust` toolchain configuration ([e3e3d0d][49])
- **gitpod:** add `bootstrap` task ([d2ce4f1][50])
- **gitpod:** added more tools to the dockerfile ([79f5f6d][51])
- minor fix ([f9bc049][52])
- minor fix ([9ae11b2][53])
- **pre-commit:** ensure Justfile is identical to upstream in `just-fmt.sh`
  script ([ee99516][54])
- **gitpod:** replace `yarn` with `npm` in Dockerfile ([927e64b][55])
- change image in `.gitpod.yml` ([f494de7][56])
- fix `.gitpod.yml` ([8c3577e][57])
...
- **api:** init ([3974f95][58])
- **api/core:** init ([891fa92][59])
- **api/errors:** init ([dc9c0cc][60])
- **api/handlers:** init ([a9354fe][61])
- **api/middlewares:** init ([03877c6][62])
- **api/registry:** init ([c093b2a][63])
- **cmd/podinfo:** init ([a3c1ebf][64])
- **internal/cryptoutil:** init ([34a4597][65])
- **internal/files:** init ([4ac385c][66])
- **internal/golang-lru:** init ([f1264aa][67])
- **internal/locksutil:** init ([09e59d1][68])
- **internal/logger:** init ([e066d10][69])
- **internal/multierror:** init ([a39b7c2][70])
- **internal/pathmanager:** init ([489996c][71])
- **internal/permitpool:** init ([a29c5d7][72])
- **internal/prettyprint:** init ([2bc4ce1][73])
- **internal/primitives:** init ([1a67ad6][74])
- **internal/runtimex:** init ([76eb234][75])
- **internal/testutils:** init ([c188be8][76])
- **internal/urandom:** init ([58ed4ff][77])
- **internal/version:** init ([4a98993][78])
- **sdk/physical:** init ([f8d487e][79])
- **internal/backoff/constant:** init ([f70b48d][80])
- **internal/backoff/exponential:** init ([437e93c][81])
- **internal/cli/data:** init ([9d115ad][82])
- **internal/cli/decoder:** init ([5b380a7][83])
- **internal/cli/flagset:** init ([a233141][84])
- **internal/cli/value:** init ([aef98ca][85])
- **internal/golang-lru/simplelru:** init ([8196569][86])
- **internal/logger/slack:** init ([1e97608][87])
- **internal/radix-tree/immutable:** init ([5dbb2e9][88])
- **internal/radix-tree/mutable:** init ([fade8c6][89])
- **sdk/api/address:** init ([69918d7][90])
- **sdk/api/fastjson:** init ([e5403e6][91])
- **sdk/api/metrics:** init ([fd5e5e2][92])
- **sdk/api/port:** init ([979298e][93])
- **sdk/api/proto:** init ([3ddbd38][94])
- **sdk/api/response:** init ([76ab9c8][95])
- **sdk/api/route:** init ([2a7f6c7][96])
- **sdk/physical/access:** init ([829e8ef][97])
- **sdk/physical/cache:** init ([48e7940][98])
- **sdk/physical/chroot:** init ([5eb4b18][99])
- **sdk/physical/encoding:** init ([b1895bf][100])
- **sdk/physical/error-injector:** init ([fa52696][101])
- **sdk/physical/latency:** init ([7d4928f][102])
- **sdk/physical/retry:** init ([8660a4d][103])
- **cmd/podinfo/commands/server:** init ([540b954][104])
- **cmd/podinfo/commands/version:** init ([df8d8d1][105])
- **docker/release:** minimal `Dockerfile` for the binary ([c4cd6da][106])

#### Documentation

- init ([93e80f2][107])
- **build/go:** `Build` target synopsis ([a9e2bfb][108])
- **just/podinfo:** `run` target synopsis ([966607f][109])
- **just/podinfo:** `kill` target synopsis ([d4bb819][110])
- **just/go:** `build-go` target synopsis ([89c2bae][111])
- **just/go:** `clean-go` target synopsis ([7089f0b][112])
- **just/go:** `lint-go` target synopsis ([25b8479][113])
- **just/go:** `format-go` target synopsis ([1c62db8][114])
- **just/go:** `bootstrap-go` target synopsis ([e37bcd0][115])
- **just/go:** `_build` target synopsis ([50231ce][116])
- **just/go:** `_go` target synopsis ([f240c28][117])
- **just/go:** `_lint-go` target synopsis ([5ac43ba][118])
- **just/semver:** `bootstrap-semver` target synopsis ([92ec6b4][119])
- **just/markdown:** `_format-markdown` target synopsis ([2721074][120])
- **just/justfile:** `format-just` target synopsis ([78a7965][121])
- **just/json:** `format-json` target synopsis ([8c5778c][122])
- **just/json:** `_format-json` target synopsis ([6a417bd][123])
- **just/bash:** `lint-bash` target synopsis ([2e74c8d][124])
- **just/bash:** `format-bash` target synopsis ([b59a5d7][125])
- **just/bash:** `_lint-bash` target synopsis ([08b1ac6][126])
- **just/bash:** `_format-bash` target synopsis ([ef340bb][127])
- **just/misc:** `snapshot` target synopsis ([4026b9e][128])
- **just/misc:** `vscode-tasks` target synopsis ([deb5e1d][129])
- **just/semver:** `generate-changelog` target synopsis ([aa5683b][130])
- **just/semver:** `patch-release` target synopsis ([74afa35][131])
- **just/semver:** `minor-release` target synopsis ([2bde4d8][132])
- **just/semver:** `major-release` target synopsis ([03bb13d][133])
- **just/semver:** variable synopsis ([a5db8af][134])
- **just/commit:** `commit` target synopsis ([d02b807][135])
- **just/commit:** `bootstrap-pre-commit` target synopsis ([c367eae][136])
- **just/commit:** `_pre-commit` target synopsis ([6aee1b8][137])
- **just/git:** `git-add` target synopsis ([f84a9eb][138])
- **just/git:** `_git-delta` target synopsis ([3781d54][139])
- **just/git:** `_git-delta` target synopsis ([b8d961d][140])
- **just/bootstrap:** `_update-rust` target synopsis ([0d1588e][141])
- **just/bootstrap:** `_validate-rust` target synopsis ([b223255][142])
- **just/bootstrap:** `_install-nodejs-package` target synopsis
  ([58aabd6][143])
- **just/bootstrap:** `_bootstrap-nodejs` target synopsis ([00df92c][144])
- **just/bootstrap:** `_core-pkgs` target synopsis ([9a78d0a][145])
- **just/bootstrap:** `_install-os-package` target synopsis ([b817954][146])
- **just/bootstrap:** `_update-os-pkgs` target synopsis ([4b94227][147])
- **just/bootstrap:** `kary-comments` target synopsis ([f80992f][148])
- **just:** common variables ([9cc3843][149])
- **build/go/targets/test:** `Target` function synopsis ([7a17aad][150])
- **build/go/targets:** package synopsis ([40e593d][151])
- **build/go/targets/test:** package synopsis ([7e5375b][152])
- **build/go/targets/build:** package synopsis ([960062d][153])
- **build/go/git:** package synopsis ([2a1ffd4][154])
- **build/go/version:** package synopsis ([265da8c][155])
- **api:** synopsis ([295aad0][156])
- **sdk:** synopsis ([9f0d098][157])
- **api/core:** synopsis ([9ef7b34][158])
- **api/handlers:** synopsis ([b0e0874][159])
- **api/middlewares:** synopsis ([d3233c8][160])
- **api/registry:** synopsis ([e801ead][161])
- **internal/files:** synopsis ([a163f57][162])
- **internal/golang-lru:** synopsis ([d63c0a7][163])
- **internal/logger:** synopsis ([5d65330][164])
- **internal/permitpool:** synopsis ([41feade][165])
- **internal/prettyprint:** synopsis ([87c51ba][166])
- **internal/primitives:** synopsis ([8a083ac][167])
- **internal/runtimex:** synopsis ([dffbbdc][168])
- **internal/testutils:** synopsis ([3895ee7][169])
- **internal/urandom:** synopsis ([ab5c0b9][170])
- **sdk/api:** synopsis ([f657f44][171])
- **sdk/physical:** synopsis ([9706c76][172])
- **cmd/podinfo/commands:** synopsis ([108d9eb][173])
- **internal/cli/data:** synopsis ([3574a9c][174])
- **internal/cli/decoder:** synopsis ([a13ad40][175])
- **internal/cli/flagset:** synopsis ([0a9d0c1][176])
- **internal/logger/slack:** synopsis ([83a078c][177])
- **internal/radix-tree/immutable:** synopsis ([78abe82][178])
- **internal/radix-tree/mutable:** synopsis ([4e53b91][179])
- **sdk/api/address:** synopsis ([23ab44b][180])
- **sdk/api/fastjson:** synopsis ([2a7c790][181])
- **sdk/api/metrics:** synopsis ([c505728][182])
- **sdk/api/port:** synopsis ([1090e11][183])
- **sdk/api/proto:** synopsis ([197fad7][184])
- **sdk/api/response:** synopsis ([5cdbfab][185])
- **sdk/api/route:** synopsis ([6d23ed6][186])
- **sdk/physical/access:** synopsis ([cea7bee][187])
- **sdk/physical/cache:** synopsis ([13e7f7d][188])
- **sdk/physical/chroot:** synopsis ([4b31bb4][189])
- **sdk/physical/encoding:** synopsis ([49a1e33][190])
- **sdk/physical/error-injector:** synopsis ([70446b1][191])
- **sdk/physical/latency:** synopsis ([1f906c5][192])
- **sdk/physical/retry:** synopsis ([13983aa][193])
- **cmd/podinfo/commands/server:** synopsis ([942978f][194])
- **cmd/podinfo/commands/version:** synopsis ([26ea212][195])
- **docker-bake:** usage guide ([5344b78][196])
- **docker-bake:** `release` target synopsis ([e55beca][197])
- **docker-bake:** `default` group synopsis ([baf2270][198])
- **docker-bake:** `TAG` variable synopsis ([abe018f][199])
- **docker-bake:** `AMD64` variable synopsis ([7909900][200])
- **docker-bake:** `ARM64` variable synopsis ([750e36d][201])
- **docker-bake:** `REGISTRY_USERNAME` variable synopsis ([4c68a04][202])
- **docker-bake:** `REGISTRY_HOSTNAME` variable synopsis ([83e5534][203])
- **docker-bake:** `LOCAL` variable synopsis ([e238d0e][204])
- **github:** `programming_task` issue template ([dba5a78][205])
- **github:** `feature_request` issue template ([4124f12][206])
- **github:** `bug_report` issue template ([c7d8513][207])
- **github:** `api_endpoint_spec` issue template ([d60a2a9][208])

#### Refactors

- **mage:** remove old files ([eabe9ea][209])

#### Tests

- **api/core:** init ([0598dda][210])
- **internal/files:** init ([ffb9e6b][211])
- **internal/logger:** init ([1d6b435][212])
- **internal/primitives:** init ([b3ebabc][213])
- **internal/backoff/constant:** init ([c3255c2][214])
- **internal/backoff/exponential:** init ([588c83a][215])
- **internal/cli/decoder:** init ([1b3cfbe][216])
- **internal/cli/value:** init ([4aeaebb][217])
- **sdk/api/fastjson:** init ([01cd7be][218])

#### Build System

- **just/core:** aggregator targets ([3382f77][219])
- **just/podinfo:** `run` target implementation ([e262bfa][220])
- **just/podinfo:** `kill` target implementation ([40a0637][221])
- **just/go:** `build-go` target implementation ([d8670aa][222])
- **just/go:** `clean-go` target implementation ([8dc7e7c][223])
- **just/go:** `lint-go` target implementation ([14209fd][224])
- **just/go:** `format-go` target implementation ([0126826][225])
- **just/go:** `bootstrap-go` target implementation ([39b68ce][226])
- **just/go:** `_build` hidden target implementation ([47dc4e8][227])
- **just/go:** `_go` hidden target implementation ([26a247a][228])
- **just/go:** `_lint-go` hidden target implementation ([351a32a][229])
- **just/semver:** `bootstrap-semver` target implementation ([3ca58f4][230])
- **just/markdown:** `_format-markdown` hidden target implementation
  ([852e53e][231])
- **just/justfile:** `format-just` target implementation ([eb8efa2][232])
- **just/json:** `format-json` target implementation ([e540c34][233])
- **just/json:** `_format-json` hidden target implementation ([2754846][234])
- **just/bash:** `lint-bash` target implementation ([5cfd1f9][235])
- **just/bash:** `format-bash` target implementation ([05d97b0][236])
- **just/bash:** `_lint-bash` hidden target implementation ([2e56839][237])
- **just/bash:** `_format-bash` hidden target implementation ([2214242][238])
- **just/misc:** `snapshot` target implementation ([184f86e][239])
- **just/misc:** `vscode-tasks` target implementation ([8669e48][240])
- **just/semver:** `generate-changelog` target implementation ([d9813b6][241])
- **just/semver:** `patch-release` target implementation ([0ee99d8][242])
- **just/semver:** `minor-release` target implementation ([47c8d9d][243])
- **just/semver:** `major-release` target implementation ([4ca9c5c][244])
- **just/semver:** variable declaration ([fb9c78a][245])
- **just/commit:** `commit` target implementation ([f574bf3][246])
- **just/commit:** `bootstrap-pre-commit` target implementation
  ([1243472][247])
- **just/git:** `_pre-commit` hidden target implementation ([e5c5206][248])
- **just/git:** `git-add` target implementation ([b053a64][249])
- **just/git:** `git-fetch` target implementation ([28764fb][250])
- **just/git:** `_git-delta` hidden target implementation ([33b956f][251])
...
- **just/bootstrap:** `_update-os-pkgs` hidden target implementation
  ([7fb7d14][259])
- **just/bootstrap:** `kary-comments` target implementation ([8ffe083][260])
- **just:** common config ([86033e0][261])
- **go:** moved from `magefile.go` ([52e7886][262])
- **go/targets/test:** moved from `mage/test` ([84ef4a7][263])
- **go/targets/build:** moved from `mage/build` ([a07a063][264])
- **go/git:** moved from `mage/git` ([ae05d7a][265])
- **go/version:** moved from `internal/version` ([1906bdc][266])
- **docker-bake:** `release` target implementation ([778b6f7][267])
- **docker-bake:** `TAG` variable declaration ([dd58811][268])
- **docker-bake:** `AMD64` variable declaration ([763dcb7][269])
- **docker-bake:** `ARM64` variable declaration ([7feeae8][270])
- **docker-bake:** `REGISTRY_USERNAME` variable declaration ([c0493f8][271])
- **docker-bake:** `REGISTRY_HOSTNAME` variable declaration ([0cebd31][272])
- **docker-bake:** `LOCAL` variable declaration ([a89e0be][273])
- **docker/release:** added buildx installer/setup to the script
  ([607964a][274])
- **docker/release:** multi-arch builder script ([2d320e6][275])
- `go-releaser` config ([25bea21][276])
- **mage:** `magefile` init ([2e7af35][277])
- **test:** `test` mage targets ([84e27ca][278])
- **build:** `build` mage targets ([5471c34][279])
- **git:** `git` auxiliary library ([4908743][280])

#### CI

- **github:** `release` workflow ([fc1e8f4][281])
- **github:** `go` workflow ([299d59e][282])
- **github:** `docker` workflow ([4e9b676][283])

#### Chore

- **pre-commit:** `just-fmt` pre-commit hook ([57b16f2][284])
- **pre-commit:** `md-fmt` pre-commit hook ([8c4b6c4][285])
- **docs:** fix `markdown` file exception `.gitignore` ([b501fa4][286])
- **build:** local `.gitignore` ([755f5d2][287])
- **internal:** local `.gitignore` ([825e7c2][288])
- **sdk:** local `.gitignore` ([f55402b][289])
- **cmd:** local `.gitignore` ([6ff23fe][290])
- **api:** local `.gitignore` ([a160290][291])
- **docs:** local `.gitignore` ([fbfd5b7][292])
- **assets:** local `.gitignore` ([502e7a5][293])
- **fixtures:** local `.gitignore` ([7a098f4][294])
- simplify `.editorconfig` file ([b61d49c][295])
- fixed `.editorconfig` file ([a8db8f7][296])
- **pre-commit:** `go-mod-tidy` pre-commit hook ([713d5ab][297])
- **pre-commit:** `go-fmt` pre-commit hook ([2fbb351][298])
- **github:** `CODEOWNERS` file ([7d5618d][299])
- **vscode:** `tasks` configuration file ([8d9e1cd][300])
- **vscode:** `settings` configuration file ([3ec3faf][301])
- **vscode:** `launch` configuration file ([7ab2d30][302])
- **vscode:** `extensions` configuration file ([2f05faf][303])
- `gitpod` config ([6aeea05][304])
- go vendor dependency management with `tools.go` ([82fea78][305])
- `.env` file init ([24c7238][306])
- `pre-commit` config file ([3d8d786][307])
- **linter:** `cspell` golang dictionary ([7b20441][308])
- **linter:** `cspell` generic dictionary ([b95cfbe][309])
- **linter:** `cspell` config file ([63d43ce][310])
- **linter:** `golangci` config file ([87a9658][311])
- **linter:** `revive` config file ([40abb30][312])
- **linter:** `.markdownlintignore` file ([eb1bae8][313])
- `.versionrc` file ([bee9668][314])
- `.stignore` file ([5bb284d][315])
- `.editorconfig` file ([640d343][316])
- `.dockerignore` file ([aabfa50][317])
- `gitignore` file ([8c88606][318])
- **linter:** `commitlint` config ([4f34502][319])

[1]: https://github.com/da-moon/northern-labs-interview/compare/v0.1.0...v0.1.1
[2]:
...
[4]: https://github.com/da-moon/northern-labs-interview/compare/v0.0.1...v0.1.0
[5]:
