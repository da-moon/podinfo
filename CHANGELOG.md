...
## [Unreleased][1] (2023-02-13)

### Build System

- **just/git:** prune refs in `git-fetch` recipe ([69d5e05][2])

## [v0.1.0][3] (2023-02-13)

### Features

- **sdk/api/response:** `WriteJSON()` function implementation ([ac8c96c][4])
- **sdk/api/response:** `WriteSuccessfulJSONRaw()` function implementation
  ([f264be1][5])
- **api/handlers:** remove `Prefix` from pre-flight handler ([ae48e9d][6])
- **api/handlers:** Register and initialize `kubernetes-liveness-probe` route
  ([1c416f5][7])
- **api/handlers/liveness:** `Initialize()` function ([73548a0][8])
- **api/handlers/liveness:** `New()` function ([016f07c][9])
- **api/handlers/liveness:** `Handler` struct ([133a5a8][10])
- **api/handlers/liveness:** package constants ([a0b8f71][11])
- **api/handlers/liveness:** `Response` struct ([9bc1341][12])

### Bug Fixes

- **sdk/api/response:** address linter complaints ([a9eed4e][13])

### Documentation

- **docker/buildx:** `gitpod` target ([11b8055][14])
- `liveness-probe` instruction ([fc5852c][15])
- **sdk/api/response:** `WriteJSON()` function synopsis ([7e25e0a][16])
- **sdk/api/response:** `WriteSuccessfulJSONRaw()` synopsis ([63d16c4][17])
- **swaggger:** add `GET` verb for `/healthz` ([5a0bb54][18])
- fix instruction to run mage's `build` target ([d35f513][19])
- **api/handlers/liveness:** `Initialize()` function ([a8993b7][20])
- **api/handlers/liveness:** `New()` function ([add5240][21])
- **api/handlers/liveness:** `Handler` struct ([06f7e41][22])
- **api/handlers/liveness:** package constants ([0e5f809][23])
- **api/handlers/liveness:** `Status` enum ([2ac0c5e][24])
- **api/handlers/liveness:** package synopsis ([efe22ac][25])
- **changelog:** updated changelog for v0.0.1 ([533f465][26])

### Refactors

- **api/handlers/liveness:** update `handler()` to write response json without
  adding additional fields ([985c2cf][27])
...
- **go-releaser:** do not sign archive ([b931100][30])
- **docker:** `gitpod` target ([006af3c][31])
- **just/bootstrap:** add `rust` toolchain ([6d9e725][32])
- **docker:** laxer package versioning in dockerfile ([9db2bb8][33])
- **docker:** fixed minor issues with `docker-bake.hcl` file ([747b4f4][34])
- **just/bootstrap:** add `bootstrap-os-pkgs` dependency to `bootstrap` target
  ([c397414][35])
- **just/go:** fix `mage` install in `_build-go` target ([7a6a5d9][36])
- **just/commit:** fix `bootstrap-pre-commit` hook installation ([495f420][37])
- **just/semver:** changed changelog commit message pattern ([9a5964e][38])

### CI

- **docker:** detect `TAG` ([5ca9bde][39])
- **go:** add `go-releaser` step ([74090a2][40])
- **release:** fix deprecated flags in `go-releaser` step ([0c641be][41])
- update min go version to `1.20` ([385d829][42])
- **go:** fixed arguments in `test` step ([2e45224][43])
- **go:** fixed arguments in `build` step ([55e6ae7][44])
- **release:** remove `unshallow` step ([2011e4f][45])
- **release:** update `setup-go` and `goreleaser-action` actions
  ([ea4cc92][46])

### Chore

- **gitpod:** source `gitpod` variables ([168e63f][47])
- **gitpod:** `rust` toolchain configuration ([e3e3d0d][48])
- **gitpod:** add `bootstrap` task ([d2ce4f1][49])
- **gitpod:** added more tools to the dockerfile ([79f5f6d][50])
- minor fix ([f9bc049][51])
- minor fix ([9ae11b2][52])
- **pre-commit:** ensure Justfile is identical to upstream in `just-fmt.sh`
  script ([ee99516][53])
- **gitpod:** replace `yarn` with `npm` in Dockerfile ([927e64b][54])
- change image in `.gitpod.yml` ([f494de7][55])
- fix `.gitpod.yml` ([8c3577e][56])
...
- **api:** init ([3974f95][57])
- **api/core:** init ([891fa92][58])
- **api/errors:** init ([dc9c0cc][59])
- **api/handlers:** init ([a9354fe][60])
- **api/middlewares:** init ([03877c6][61])
- **api/registry:** init ([c093b2a][62])
- **cmd/podinfo:** init ([a3c1ebf][63])
- **internal/cryptoutil:** init ([34a4597][64])
- **internal/files:** init ([4ac385c][65])
- **internal/golang-lru:** init ([f1264aa][66])
- **internal/locksutil:** init ([09e59d1][67])
- **internal/logger:** init ([e066d10][68])
- **internal/multierror:** init ([a39b7c2][69])
- **internal/pathmanager:** init ([489996c][70])
- **internal/permitpool:** init ([a29c5d7][71])
- **internal/prettyprint:** init ([2bc4ce1][72])
- **internal/primitives:** init ([1a67ad6][73])
- **internal/runtimex:** init ([76eb234][74])
- **internal/testutils:** init ([c188be8][75])
- **internal/urandom:** init ([58ed4ff][76])
- **internal/version:** init ([4a98993][77])
- **sdk/physical:** init ([f8d487e][78])
- **internal/backoff/constant:** init ([f70b48d][79])
- **internal/backoff/exponential:** init ([437e93c][80])
- **internal/cli/data:** init ([9d115ad][81])
- **internal/cli/decoder:** init ([5b380a7][82])
- **internal/cli/flagset:** init ([a233141][83])
- **internal/cli/value:** init ([aef98ca][84])
- **internal/golang-lru/simplelru:** init ([8196569][85])
- **internal/logger/slack:** init ([1e97608][86])
- **internal/radix-tree/immutable:** init ([5dbb2e9][87])
- **internal/radix-tree/mutable:** init ([fade8c6][88])
- **sdk/api/address:** init ([69918d7][89])
- **sdk/api/fastjson:** init ([e5403e6][90])
- **sdk/api/metrics:** init ([fd5e5e2][91])
- **sdk/api/port:** init ([979298e][92])
- **sdk/api/proto:** init ([3ddbd38][93])
- **sdk/api/response:** init ([76ab9c8][94])
- **sdk/api/route:** init ([2a7f6c7][95])
- **sdk/physical/access:** init ([829e8ef][96])
- **sdk/physical/cache:** init ([48e7940][97])
- **sdk/physical/chroot:** init ([5eb4b18][98])
- **sdk/physical/encoding:** init ([b1895bf][99])
- **sdk/physical/error-injector:** init ([fa52696][100])
- **sdk/physical/latency:** init ([7d4928f][101])
- **sdk/physical/retry:** init ([8660a4d][102])
- **cmd/podinfo/commands/server:** init ([540b954][103])
- **cmd/podinfo/commands/version:** init ([df8d8d1][104])
- **docker/release:** minimal `Dockerfile` for the binary ([c4cd6da][105])

#### Documentation

- init ([93e80f2][106])
- **build/go:** `Build` target synopsis ([a9e2bfb][107])
- **just/podinfo:** `run` target synopsis ([966607f][108])
- **just/podinfo:** `kill` target synopsis ([d4bb819][109])
- **just/go:** `build-go` target synopsis ([89c2bae][110])
- **just/go:** `clean-go` target synopsis ([7089f0b][111])
- **just/go:** `lint-go` target synopsis ([25b8479][112])
- **just/go:** `format-go` target synopsis ([1c62db8][113])
- **just/go:** `bootstrap-go` target synopsis ([e37bcd0][114])
- **just/go:** `_build` target synopsis ([50231ce][115])
- **just/go:** `_go` target synopsis ([f240c28][116])
- **just/go:** `_lint-go` target synopsis ([5ac43ba][117])
- **just/semver:** `bootstrap-semver` target synopsis ([92ec6b4][118])
- **just/markdown:** `_format-markdown` target synopsis ([2721074][119])
- **just/justfile:** `format-just` target synopsis ([78a7965][120])
- **just/json:** `format-json` target synopsis ([8c5778c][121])
- **just/json:** `_format-json` target synopsis ([6a417bd][122])
- **just/bash:** `lint-bash` target synopsis ([2e74c8d][123])
- **just/bash:** `format-bash` target synopsis ([b59a5d7][124])
- **just/bash:** `_lint-bash` target synopsis ([08b1ac6][125])
- **just/bash:** `_format-bash` target synopsis ([ef340bb][126])
- **just/misc:** `snapshot` target synopsis ([4026b9e][127])
- **just/misc:** `vscode-tasks` target synopsis ([deb5e1d][128])
- **just/semver:** `generate-changelog` target synopsis ([aa5683b][129])
- **just/semver:** `patch-release` target synopsis ([74afa35][130])
- **just/semver:** `minor-release` target synopsis ([2bde4d8][131])
- **just/semver:** `major-release` target synopsis ([03bb13d][132])
- **just/semver:** variable synopsis ([a5db8af][133])
- **just/commit:** `commit` target synopsis ([d02b807][134])
- **just/commit:** `bootstrap-pre-commit` target synopsis ([c367eae][135])
- **just/commit:** `_pre-commit` target synopsis ([6aee1b8][136])
- **just/git:** `git-add` target synopsis ([f84a9eb][137])
- **just/git:** `_git-delta` target synopsis ([3781d54][138])
- **just/git:** `_git-delta` target synopsis ([b8d961d][139])
- **just/bootstrap:** `_update-rust` target synopsis ([0d1588e][140])
- **just/bootstrap:** `_validate-rust` target synopsis ([b223255][141])
- **just/bootstrap:** `_install-nodejs-package` target synopsis
  ([58aabd6][142])
- **just/bootstrap:** `_bootstrap-nodejs` target synopsis ([00df92c][143])
- **just/bootstrap:** `_core-pkgs` target synopsis ([9a78d0a][144])
- **just/bootstrap:** `_install-os-package` target synopsis ([b817954][145])
- **just/bootstrap:** `_update-os-pkgs` target synopsis ([4b94227][146])
- **just/bootstrap:** `kary-comments` target synopsis ([f80992f][147])
- **just:** common variables ([9cc3843][148])
- **build/go/targets/test:** `Target` function synopsis ([7a17aad][149])
- **build/go/targets:** package synopsis ([40e593d][150])
- **build/go/targets/test:** package synopsis ([7e5375b][151])
- **build/go/targets/build:** package synopsis ([960062d][152])
- **build/go/git:** package synopsis ([2a1ffd4][153])
- **build/go/version:** package synopsis ([265da8c][154])
- **api:** synopsis ([295aad0][155])
- **sdk:** synopsis ([9f0d098][156])
- **api/core:** synopsis ([9ef7b34][157])
- **api/handlers:** synopsis ([b0e0874][158])
- **api/middlewares:** synopsis ([d3233c8][159])
- **api/registry:** synopsis ([e801ead][160])
- **internal/files:** synopsis ([a163f57][161])
- **internal/golang-lru:** synopsis ([d63c0a7][162])
- **internal/logger:** synopsis ([5d65330][163])
- **internal/permitpool:** synopsis ([41feade][164])
- **internal/prettyprint:** synopsis ([87c51ba][165])
- **internal/primitives:** synopsis ([8a083ac][166])
- **internal/runtimex:** synopsis ([dffbbdc][167])
- **internal/testutils:** synopsis ([3895ee7][168])
- **internal/urandom:** synopsis ([ab5c0b9][169])
- **sdk/api:** synopsis ([f657f44][170])
- **sdk/physical:** synopsis ([9706c76][171])
- **cmd/podinfo/commands:** synopsis ([108d9eb][172])
- **internal/cli/data:** synopsis ([3574a9c][173])
- **internal/cli/decoder:** synopsis ([a13ad40][174])
- **internal/cli/flagset:** synopsis ([0a9d0c1][175])
- **internal/logger/slack:** synopsis ([83a078c][176])
- **internal/radix-tree/immutable:** synopsis ([78abe82][177])
- **internal/radix-tree/mutable:** synopsis ([4e53b91][178])
- **sdk/api/address:** synopsis ([23ab44b][179])
- **sdk/api/fastjson:** synopsis ([2a7c790][180])
- **sdk/api/metrics:** synopsis ([c505728][181])
- **sdk/api/port:** synopsis ([1090e11][182])
- **sdk/api/proto:** synopsis ([197fad7][183])
- **sdk/api/response:** synopsis ([5cdbfab][184])
- **sdk/api/route:** synopsis ([6d23ed6][185])
- **sdk/physical/access:** synopsis ([cea7bee][186])
- **sdk/physical/cache:** synopsis ([13e7f7d][187])
- **sdk/physical/chroot:** synopsis ([4b31bb4][188])
- **sdk/physical/encoding:** synopsis ([49a1e33][189])
- **sdk/physical/error-injector:** synopsis ([70446b1][190])
- **sdk/physical/latency:** synopsis ([1f906c5][191])
- **sdk/physical/retry:** synopsis ([13983aa][192])
- **cmd/podinfo/commands/server:** synopsis ([942978f][193])
- **cmd/podinfo/commands/version:** synopsis ([26ea212][194])
- **docker-bake:** usage guide ([5344b78][195])
- **docker-bake:** `release` target synopsis ([e55beca][196])
- **docker-bake:** `default` group synopsis ([baf2270][197])
- **docker-bake:** `TAG` variable synopsis ([abe018f][198])
- **docker-bake:** `AMD64` variable synopsis ([7909900][199])
- **docker-bake:** `ARM64` variable synopsis ([750e36d][200])
- **docker-bake:** `REGISTRY_USERNAME` variable synopsis ([4c68a04][201])
- **docker-bake:** `REGISTRY_HOSTNAME` variable synopsis ([83e5534][202])
- **docker-bake:** `LOCAL` variable synopsis ([e238d0e][203])
- **github:** `programming_task` issue template ([dba5a78][204])
- **github:** `feature_request` issue template ([4124f12][205])
- **github:** `bug_report` issue template ([c7d8513][206])
- **github:** `api_endpoint_spec` issue template ([d60a2a9][207])

#### Refactors

- **mage:** remove old files ([eabe9ea][208])

#### Tests

- **api/core:** init ([0598dda][209])
- **internal/files:** init ([ffb9e6b][210])
- **internal/logger:** init ([1d6b435][211])
- **internal/primitives:** init ([b3ebabc][212])
- **internal/backoff/constant:** init ([c3255c2][213])
- **internal/backoff/exponential:** init ([588c83a][214])
- **internal/cli/decoder:** init ([1b3cfbe][215])
- **internal/cli/value:** init ([4aeaebb][216])
- **sdk/api/fastjson:** init ([01cd7be][217])

#### Build System

- **just/core:** aggregator targets ([3382f77][218])
- **just/podinfo:** `run` target implementation ([e262bfa][219])
- **just/podinfo:** `kill` target implementation ([40a0637][220])
- **just/go:** `build-go` target implementation ([d8670aa][221])
- **just/go:** `clean-go` target implementation ([8dc7e7c][222])
- **just/go:** `lint-go` target implementation ([14209fd][223])
- **just/go:** `format-go` target implementation ([0126826][224])
- **just/go:** `bootstrap-go` target implementation ([39b68ce][225])
- **just/go:** `_build` hidden target implementation ([47dc4e8][226])
- **just/go:** `_go` hidden target implementation ([26a247a][227])
- **just/go:** `_lint-go` hidden target implementation ([351a32a][228])
- **just/semver:** `bootstrap-semver` target implementation ([3ca58f4][229])
- **just/markdown:** `_format-markdown` hidden target implementation
  ([852e53e][230])
- **just/justfile:** `format-just` target implementation ([eb8efa2][231])
- **just/json:** `format-json` target implementation ([e540c34][232])
- **just/json:** `_format-json` hidden target implementation ([2754846][233])
- **just/bash:** `lint-bash` target implementation ([5cfd1f9][234])
- **just/bash:** `format-bash` target implementation ([05d97b0][235])
- **just/bash:** `_lint-bash` hidden target implementation ([2e56839][236])
- **just/bash:** `_format-bash` hidden target implementation ([2214242][237])
- **just/misc:** `snapshot` target implementation ([184f86e][238])
- **just/misc:** `vscode-tasks` target implementation ([8669e48][239])
- **just/semver:** `generate-changelog` target implementation ([d9813b6][240])
- **just/semver:** `patch-release` target implementation ([0ee99d8][241])
- **just/semver:** `minor-release` target implementation ([47c8d9d][242])
- **just/semver:** `major-release` target implementation ([4ca9c5c][243])
- **just/semver:** variable declaration ([fb9c78a][244])
- **just/commit:** `commit` target implementation ([f574bf3][245])
- **just/commit:** `bootstrap-pre-commit` target implementation
  ([1243472][246])
- **just/git:** `_pre-commit` hidden target implementation ([e5c5206][247])
- **just/git:** `git-add` target implementation ([b053a64][248])
- **just/git:** `git-fetch` target implementation ([28764fb][249])
- **just/git:** `_git-delta` hidden target implementation ([33b956f][250])
...
- **just/bootstrap:** `_update-os-pkgs` hidden target implementation
  ([7fb7d14][258])
- **just/bootstrap:** `kary-comments` target implementation ([8ffe083][259])
- **just:** common config ([86033e0][260])
- **go:** moved from `magefile.go` ([52e7886][261])
- **go/targets/test:** moved from `mage/test` ([84ef4a7][262])
- **go/targets/build:** moved from `mage/build` ([a07a063][263])
- **go/git:** moved from `mage/git` ([ae05d7a][264])
- **go/version:** moved from `internal/version` ([1906bdc][265])
- **docker-bake:** `release` target implementation ([778b6f7][266])
- **docker-bake:** `TAG` variable declaration ([dd58811][267])
- **docker-bake:** `AMD64` variable declaration ([763dcb7][268])
- **docker-bake:** `ARM64` variable declaration ([7feeae8][269])
- **docker-bake:** `REGISTRY_USERNAME` variable declaration ([c0493f8][270])
- **docker-bake:** `REGISTRY_HOSTNAME` variable declaration ([0cebd31][271])
- **docker-bake:** `LOCAL` variable declaration ([a89e0be][272])
- **docker/release:** added buildx installer/setup to the script
  ([607964a][273])
- **docker/release:** multi-arch builder script ([2d320e6][274])
- `go-releaser` config ([25bea21][275])
- **mage:** `magefile` init ([2e7af35][276])
- **test:** `test` mage targets ([84e27ca][277])
- **build:** `build` mage targets ([5471c34][278])
- **git:** `git` auxiliary library ([4908743][279])

#### CI

- **github:** `release` workflow ([fc1e8f4][280])
- **github:** `go` workflow ([299d59e][281])
- **github:** `docker` workflow ([4e9b676][282])

#### Chore

- **pre-commit:** `just-fmt` pre-commit hook ([57b16f2][283])
- **pre-commit:** `md-fmt` pre-commit hook ([8c4b6c4][284])
- **docs:** fix `markdown` file exception `.gitignore` ([b501fa4][285])
- **build:** local `.gitignore` ([755f5d2][286])
- **internal:** local `.gitignore` ([825e7c2][287])
- **sdk:** local `.gitignore` ([f55402b][288])
- **cmd:** local `.gitignore` ([6ff23fe][289])
- **api:** local `.gitignore` ([a160290][290])
- **docs:** local `.gitignore` ([fbfd5b7][291])
- **assets:** local `.gitignore` ([502e7a5][292])
- **fixtures:** local `.gitignore` ([7a098f4][293])
- simplify `.editorconfig` file ([b61d49c][294])
- fixed `.editorconfig` file ([a8db8f7][295])
- **pre-commit:** `go-mod-tidy` pre-commit hook ([713d5ab][296])
- **pre-commit:** `go-fmt` pre-commit hook ([2fbb351][297])
- **github:** `CODEOWNERS` file ([7d5618d][298])
- **vscode:** `tasks` configuration file ([8d9e1cd][299])
- **vscode:** `settings` configuration file ([3ec3faf][300])
- **vscode:** `launch` configuration file ([7ab2d30][301])
- **vscode:** `extensions` configuration file ([2f05faf][302])
- `gitpod` config ([6aeea05][303])
- go vendor dependency management with `tools.go` ([82fea78][304])
- `.env` file init ([24c7238][305])
- `pre-commit` config file ([3d8d786][306])
- **linter:** `cspell` golang dictionary ([7b20441][307])
- **linter:** `cspell` generic dictionary ([b95cfbe][308])
- **linter:** `cspell` config file ([63d43ce][309])
- **linter:** `golangci` config file ([87a9658][310])
- **linter:** `revive` config file ([40abb30][311])
- **linter:** `.markdownlintignore` file ([eb1bae8][312])
- `.versionrc` file ([bee9668][313])
- `.stignore` file ([5bb284d][314])
- `.editorconfig` file ([640d343][315])
- `.dockerignore` file ([aabfa50][316])
- `gitignore` file ([8c88606][317])
- **linter:** `commitlint` config ([4f34502][318])

[1]: https://github.com/da-moon/northern-labs-interview/compare/v0.1.0...HEAD
[2]:
  https://github.com/da-moon/northern-labs-interview/commit/69d5e05a3d9f059fa608adf9d7d0915f96eb5a2b
[3]: https://github.com/da-moon/northern-labs-interview/compare/v0.0.1...v0.1.0
[4]:
